...
// The purpose of this program is to patch GuiltyGearXrd.exe to replace instructions in it
// related to the game interacting with DirectInput8 API, so that game controllers may work
// and their inputs may be processed by the game, even while the game's window is in the background
#include <iostream>
#include <string>
#ifndef FOR_LINUX
#include "ConsoleEmulator.h"
#include "GGXrdBackgroundGamepad.h"
...
#define CrossPlatformStringCompareCaseInsensitive _wcsicmp
#else
...
#define CrossPlatformStringCompareCaseInsensitive strcasecmp
...
struct StringWithLength {
	...
	inline StringWithLength(const char(&text)[size]) : len(size - 1), text(text) { }
};
...
void meatOfTheProgram() {
    ...
    for (const Section& section : sections) {
    	if (section.name == ".text") {
    		textBegin = wholeFileBegin + section.rawAddress;
	...
	int sigRes = sigscan(textBegin, textEnd,
		"\x8b\x13\x8b\x42\x24\xff\xd0\x85\xc0",
		"xxxxxxxxx");
	if (sigRes == -1) {
        CrossPlatformCout << "Failed to find patching place 1\n";
        return;
	}
	sigRes += textBegin - wholeFileBegin;
	sigRes += 9;
	
	fseek(file, sigRes, SEEK_SET);
	// original code: 0f 84 0d 12 00 00, a 6-byte jump instruction
	// This is a check for whether the game's window is in focus right now,
	// and we're removing it.
	// Don't worry, this won't make keyboard input suddenly start working when
	// the game is not in focus. Keystrokes actually appear in another place,
	// and this function in context of which we're now only seems to be
	// confirming whether a key is still pressed.
	fwrite("\x90\x90\x90\x90\x90\x90", 1, 6, file);
	CrossPlatformCout << "Removed the jump which was checking for whether the game's window is in focus, successfully.\n";
	
	struct PatchingPlace {
		StringWithLength sig;
		int offset;
	};
	PatchingPlace places[3] {
		{ "\x6a\x06\x52\x50\x8b\x41\x34\xff\xd0", 1 },
		{ "\x8b\x41\x04\x8b\x32\x8b\x50\x1c\x83\xc1\x04\x6a\x06\xff\xd2", 12 },
		{ "\x8b\x04\x2f\x8b\x56\x74\x8b\x08\x6a\x06\x52\x50\x8b\x41\x34\xff\xd0", 9 }
	};
	std::vector<char> mask;
	
	for (int i = 0; i < 3; ++i) {
		const PatchingPlace& place = places[i];
		
		mask.resize(place.sig.len + 1);
		memset(mask.data(), 'x', place.sig.len);
		mask[place.sig.len] = '\0';
		
		sigRes = sigscan(textBegin, textEnd,
			place.sig.text,
			mask.data());
		if (sigRes == -1) {
	        CrossPlatformCout << "Failed to find patching place " << i + 1 << " out of 3\n";
	        return;
		}
		sigRes += textBegin - wholeFileBegin;
		fseek(file, sigRes + place.offset, SEEK_SET);
		fwrite("\x0e", 1, 1, file);
		CrossPlatformCout << "Added DISCL_BACKGROUND flag to the IDirectInputDevice8::SetCooperativeLevel method call (place " << i + 1 << " out of 3).\n";
	}
...
#define BASIC_EXPLANATION "This program patches " << exeName << " executable to permanently make it so that" \
                  " game controllers may continue working with the game even when its window is in the background."
...
UINT windowClassNameResourceId = IDC_GGXRDBACKGROUNDGAMEPAD;
LPCWSTR windowIconId = MAKEINTRESOURCEW(IDI_ICON1);
LPCWSTR windowMenuName = MAKEINTRESOURCEW(IDC_GGXRDBACKGROUNDGAMEPAD);
LPCWSTR windowAcceleratorId = MAKEINTRESOURCEW(IDC_GGXRDBACKGROUNDGAMEPAD);
#endif

std::string& operator<<(std::string& ref, const char* in) {
	ref += in;
	return ref;
}

bool parseArgs(int argc, CrossPlatformChar** argv, int* exitCode) {
	for (int i = 0; i < argc; ++i) {
		if (CrossPlatformStringCompareCaseInsensitive(argv[i], CrossPlatformText("/?")) == 0
				|| CrossPlatformStringCompareCaseInsensitive(argv[i], CrossPlatformText("--help")) == 0
				|| CrossPlatformStringCompareCaseInsensitive(argv[i], CrossPlatformText("-help")) == 0) {
			
			std::string builder;
			builder << BASIC_EXPLANATION;
			
			#ifndef FOR_LINUX
			MessageBoxA(NULL,
				builder.c_str(),
				"GGXrdBackgroundGamepad v" VERSION,
				MB_OK);
			#else
			CrossPlatformCout << "GGXrdBackgroundGamepad v" VERSION << '\n' << BASIC_EXPLANATION << '\n';
			#endif
			*exitCode = 0;
			return false;
		}
...
void OutputStringA(const char* str) { std::cout << str; }
void OutputStringW(const wchar_t* str) { std::wcout << str; }
int main(int argc, char** argv) {
	...
	int exitCode = 0;
	if (!parseArgs(argc, argv, &exitCode)) return exitCode;
	return patcherMain();
}
#endif
